...
public sealed class ChangePasswordByAdminHandler(
    ILogger<ChangePasswordHandler> logger,
    MailPathForUserManager path,
    IEmailSender emailService,
    AuthContext auth) : IChangePasswordByAdminHandler
{
    public async Task<Result<Message>> Handle(ChangePasswordByAdminRequest request, CancellationToken cancellationToken)
    {
        Result<ChangePasswordByAdminResult> result = await request.Ensure(async (request) =>
        ...
        if (result.Data.EmailConfirmed)
        {
            ...
            await emailService.SendMailWithTemplate(
                new MailMessageWithTemplate<MailPasswordChangeModel>(
                    ...
                    result.Data.Email, path.ChangePasswordTemplatePath,
                    new MailPasswordChangeModel(request.NewPassword, inLine)),
                [],
                cancellationToken);
        }
    ...
    private async Task<Result<ChangePasswordByAdminResult>> InnerHandle(ChangePasswordByAdminRequest request, CancellationToken cancellationToken)
    {
        ...
        if (user is null)
        {
            ...
            return CreateResult.Failure<ChangePasswordByAdminResult>(UserErrors.UserNotFound);
        }
        ...
        return CreateResult.Success(new ChangePasswordByAdminResult(user.EmailConfirmed, user.Email));
    }
}
